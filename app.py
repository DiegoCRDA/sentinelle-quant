...
# Configuration
st.set_page_config(page_title="Sentinelle v2.6 - Full Dashboard", layout="centered")
st.title("🛡️ Sentinelle Quant Pro v2.6")
...
if state == "BUY" and s4 > 7.0:
    st.success(f"MODE : ACHAT (LONG) - Cible entrée : {p_f * (1-(atr/100)):.4f} $")
elif state == "SELL" and s4 > 7.0:
    st.error(f"MODE : VENTE (SHORT) - Cible entrée : {p_f * (1+(atr/100)):.4f} $")
else: st.info("MODE : OBSERVATION")

# RESTAURATION DES MÉTRIQUES SYSTÈME
